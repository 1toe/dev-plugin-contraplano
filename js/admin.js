 ...
 * Versión 1.0.8 - Mejorado sistema de guardado y gestión de áreas
 */
(function ($) {
    'use strict';
    ...
    var VibeBookFlipAdmin = {
        ...
        init: function () {
            ...
            // Inicializar eventos de guardado
            this.initSaveEvents();
        ...
        checkShowTutorial: function() {
            // Verificar si ya se mostró antes utilizando localStorage
            var tutorialShown = localStorage.getItem('vibebook_tutorial_shown');
        ...
        showTutorial: function() {
            $('#vibebook-welcome-tutorial').addClass('active');
        },
        
        // Ocultar tutorial
        hideTutorial: function() {
            $('#vibebook-welcome-tutorial').removeClass('active');
            
            // Verificar si no mostrar de nuevo
            if ($('#vibebook-dont-show-again').is(':checked')) {
                localStorage.setItem('vibebook_tutorial_shown', 'true');
            }
        },
        
        // Inicializar eventos del tutorial
        initTutorialEvents: function() {
            var self = this;
            
            // Botón de cerrar
            $('.vibebook-tutorial-close').on('click', function() {
                self.hideTutorial();
            });
            
            // Botón de mostrar tutorial
            $('#vibebook-show-tutorial').on('click', function() {
                self.showTutorial();
            });
            
            // Botón de comenzar
            $('.vibebook-tutorial-start').on('click', function() {
                self.hideTutorial();
            });
            
            // Navegación entre pasos
            $('.vibebook-tutorial-next').on('click', function() {
            ...
            $('.vibebook-tutorial-prev').on('click', function() {
            ...
            $('.vibebook-tutorial-dot').on('click', function() {
                self.tutorialStep = parseInt($(this).data('step'));
                self.updateTutorialStep();
            });
        },
        ...
        updateTutorialStep: function() {
            ...
            $('.vibebook-tutorial-step').removeClass('active');
            
            // Mostrar paso actual
            $('.vibebook-tutorial-step[data-step="' + this.tutorialStep + '"]').addClass('active');
            
            // Actualizar puntos
            $('.vibebook-tutorial-dot').removeClass('active');
            $('.vibebook-tutorial-dot[data-step="' + this.tutorialStep + '"]').addClass('active');
            
            // Actualizar botones
            if (this.tutorialStep === 1) {
                $('.vibebook-tutorial-prev').prop('disabled', true);
            } else {
                $('.vibebook-tutorial-prev').prop('disabled', false);
            }
            
            if (this.tutorialStep === 3) {
                $('.vibebook-tutorial-next').prop('disabled', true);
            } else {
                $('.vibebook-tutorial-next').prop('disabled', false);
            }
        },

        // Inicializar eventos de guardado automático
        ...
        showSaveMessage: function(message, type) {
            var $indicator = $('#vibebook-save-indicator');
            var $message = $('#vibebook-save-message');
            
            $indicator.removeClass('success error info warning').addClass(type || 'info');
            $message.text(message);
            $indicator.show();
            
            // Ocultar mensaje después de 3 segundos
            clearTimeout(this.saveMessageTimer);
            this.saveMessageTimer = setTimeout(function() {
                $indicator.fadeOut();
            }, 3000);
        },
        ...
        loadFlipbookData: function (flipbookId) {
            ...
            // Solicitar datos del flipbook desde el servidor
            $.ajax({
                ...
                success: function (response) {
                    if (response.success) {
                        ...
                        self.showSaveMessage('Flipbook cargado correctamente', 'success');
        ...
        renderPage: function (num) {
            ...
            self.pdfDoc.getPage(num).then(function (page) {
                ...
                var renderContext = {
                    ...
                    viewport: viewport // Pasar viewportm esto significa que el canvas se ajustará al tamaño del PDF
                };
        ...
        saveAreas: function () {
            ...
            self.showSaveMessage('Guardando áreas...', 'info');

            // Crear copia de áreas con toda la información necesaria
            var areasToSave = JSON.parse(JSON.stringify(self.areas));
            
            // Verificar que todas las áreas tengan coords_percent
            areasToSave.forEach(function(area) {
                if (!area.coords_percent && area.coords && self.pdfOriginalWidth && self.pdfOriginalHeight) {
                    area.coords_percent = [
                        (area.coords[0] / self.pdfOriginalWidth) * 100,
                        (area.coords[1] / self.pdfOriginalHeight) * 100,
                        (area.coords[2] / self.pdfOriginalWidth) * 100,
            ...
            // Marcar timestamp de inicio del guardado
            self.lastSave = new Date().getTime();
            self.unsavedChanges = false;

            // Actualizar áreas en servidor mediante AJAX
            $.ajax({
                url: vibeBookFlip.ajaxUrl,
                type: 'POST',
                data: {
                    ...
                    areas: JSON.stringify(areasToSave)
                },
                success: function (response) {
                    if (response.success) {
                        // Mostrar mensaje de éxito
                        self.showSaveMessage('Áreas guardadas correctamente (' + response.data.areas_count + ' áreas)', 'success');
                    } else {
                        // Mostrar error
                        self.showSaveMessage('Error al guardar: ' + response.data, 'error');
                        self.unsavedChanges = true;
                    }
                },
                error: function () {
                    // Mostrar error de conexión
                    self.showSaveMessage('Error de conexión al guardar las áreas', 'error');
                    self.unsavedChanges = true;
                }
            });
        },
        ...
        deleteArea: function (id) {
            ...
            // Mostrar mensaje
            self.showSaveMessage('Área eliminada', 'info');
        },
